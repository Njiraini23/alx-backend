...
const listProducts = [
  ...
  { 'itemId': 4, 'itemName': 'Suitcase 1050', 'price': 550, 'initialAvailabilityQuantity': 5}
];
...
app.get('/list_products/:itemId', async function(req, res) {
  ...
  if (item) {
    ...
    const resItem = {
      ...
      currentQuantity: stock !== null ? parseInt(stock) : item.initialAvialbleQuantity,
    };
    res.json(resItem);
  } else {
...
app.get('/reserve_product/:itemId', async function (req, res) => {
  ...
  if (currentStock !== null) {
    ...
    if (currentStock > 0) {
      ...
      res.json("{status": "Not enough stock vailable", "itemId": itemId});
    }
  } else {
    reserveStockById(itemId, item.intialAvailableQuantity - 1);
    res.json({"status": "Reservation confirmed", "itemId": itemId});
  }
});
